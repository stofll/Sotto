...
Sotto is a desktop dictation application for Windows and macOS. The table below describes the support policy for the current tree; a CI build is not by itself a promise of a published installer.
...
The bundle catalogue is gated on `#[cfg(any(windows, target_os = "macos"))]`, so Windows and macOS carry the same set; there are no Windows-only entries in it. See [Models](models.md) for the families and their languages.

The macOS deployment minimum is configured in the Tauri bundle metadata. Check the current release notes before relying on a particular architecture or installer format.

Platform-specific prerequisites, permissions, and the commands contributors should use are all in [Development](development.md).
