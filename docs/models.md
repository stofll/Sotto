...
Speed initially uses measured CPU reference results for the exact model revision and language setting where available. This compares models on the reference machine; it is not a predicted time on your computer or an accuracy rating. After five comparable successful local dictations, the bar uses their median processing-to-audio ratio. The speed hint gives a plain fast/moderate/slow estimate, without benchmark statistics or machine identifiers. Cold runs and duration bands are kept separate, and inconsistent samples fall back to an available reference instead of producing a confident personal score. File transcription measurements do not influence the dictation bar. Streaming models are assessed by their final full-audio transcription pass; this does not measure live-preview latency.

When the exact context has no reference, the same model's measurement from another context stands in: the same device with the language left to the engine, or — because the catalog is measured on processors while the application asks for the GPU by default — the processor measurement. Such a bar is drawn fainter and its details say the number came from elsewhere, because a processor result is not evidence about a graphics card. Without any reference for that model the bar stays unknown until local dictations produce one.

Memory headroom uses the catalog's approximate RAM requirement and currently available system memory, reserving 20% for other work. Requirements are estimates, so a full bar is not a guarantee against allocation failure. A model already in memory counts its own footprint back into the budget, since unloading it returns that memory; its details say it is loaded. On a discrete graphics card a GPU request stays unknown rather than borrowing system RAM as VRAM: the current engine API cannot confirm the actual GPU or available VRAM. Apple Silicon shares one pool between processor and graphics, so free system memory is the budget there whichever device runs the model. The memory hint shows an approximate requirement and current free memory when available; a load failure alone is not interpreted as insufficient memory.

The catalog refreshes on opening, relevant model/configuration changes, completed measurements, and window focus. It does not continuously poll hardware. Automatic observations stay local; see [Privacy](privacy.md). Reference coverage and reproduction instructions are in [Benchmarks](benchmarks.md).
